...
struct inlineHookInfo {
    struct inlineHookItem item[1024];//�ṹ������
    int size;
};
...
registerInlineHook(uint32_t target_addr, uint32_t new_addr, uint32_t **proto_addr) {
    ...
    item->length = TEST_BIT0(item->target_addr) ? 12 : 12;
    ...
    item->trampoline_instructions =
            mmap(NULL, PAGE_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANONYMOUS | MAP_PRIVATE,
                 0, 0);
...
static void doInlineHook(struct inlineHookItem *item) {
    ...
    if (TEST_BIT0(item->target_addr)) {
        ...
        ((uint32_t *) (item->target_addr))[0] = 0xE1A00000;    // MOV R0,R0
        ((uint32_t *) (item->target_addr))[1] = 0xe51ff004;    // LDR PC, [PC, #-4]
        ((uint32_t *) (item->target_addr))[2] = item->new_addr;
    }
