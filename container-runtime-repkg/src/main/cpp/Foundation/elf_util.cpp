...
ElfImg::ElfImg(const char *elf) {
    ...
    if (fd < 0) {
        ALOGE("failed to open %s", elf);
        return;
    }

    size = lseek(fd, 0, SEEK_END);
    if (size <= 0) {
        ALOGE("lseek() failed for %s", elf);
    }
    ...
    if (!symtab_offset) {
        ALOGW("can't find symtab from sections\n");
    }
...
void *ElfImg::getModuleBase(const char *name) {
    ...
    if (!found) {
        ALOGE("failed to read load address for %s", name);
        return nullptr;
    }
    ...
    ALOGD("get module base %s: %lu", name, load_addr);
...
extern "C"
void *findSym(const char *elf, const char *sym_name) {
