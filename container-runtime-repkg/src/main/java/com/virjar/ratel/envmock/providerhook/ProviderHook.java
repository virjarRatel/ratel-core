...
//        });
    ...
    public static void registerFetcher(String authority, HookFetcher hookFetcher) {
        PROVIDER_MAP.put(authority, hookFetcher);
    }
    ...
    private static HookFetcher fetchHook(String authority) {
        ...
        if (fetcher == null) {
            fetcher = new HookFetcher() {
                @Override
                public ProviderHook fetch(boolean external, IInterface provider) {
                    return new ProviderHook(provider);
                }
            };
        }
        return fetcher;
    ...
    public Object invoke(Object proxy, Method method, Object... args) throws Throwable {
        ...
        try {
            ...
            if ("call".equals(name)) {
                ...
                Object result = call(methodBox, methodName, arg, extras);
                if (RatelRuntime.isRatelDebugBuild) {
                    Log.i(Constants.TAG, "content provider call: " + methodName
                            + " arg:" + arg + "  extras:" + extras + " result:" + result);
                }
                return result;
                ...
                if (RatelRuntime.isRatelDebugBuild) {
                    Log.i(Constants.TAG, "content provider call url: " + url +
                            " projection:" + StringUtils.join(projection, ",")
                            + " selection:" + selection + " selectionArgs:" + StringUtils.join(selectionArgs, ",")
                            + " sortOrder:" + sortOrder + " queryArgs:" + StringUtils.join(queryArgs, ","));
                }
                return query(methodBox, url, projection, selection, selectionArgs, sortOrder, queryArgs);
            }
            return methodBox.call();
        } catch (Throwable e) {
            Log.w(Constants.TAG, "ProviderHook" + String.format("call: %s (%s) with error", method.getName(), Arrays.toString(args)));
