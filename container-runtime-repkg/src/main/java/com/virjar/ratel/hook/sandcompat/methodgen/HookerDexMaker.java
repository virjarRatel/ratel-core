...
import com.virjar.ratel.hook.sandcompat.utils.DexLog;
...
import static com.virjar.ratel.hook.sandcompat.utils.DexMakerUtils.autoBoxIfNecessary;
import static com.virjar.ratel.hook.sandcompat.utils.DexMakerUtils.autoUnboxIfNecessary;
import static com.virjar.ratel.hook.sandcompat.utils.DexMakerUtils.createResultLocals;
import static com.virjar.ratel.hook.sandcompat.utils.DexMakerUtils.getObjTypeIdIfPrimitive;
...
public class HookerDexMaker implements HookMaker {
    ...
    private static final TypeId<RC_MethodHook> callbackTypeId = TypeId.get(RC_MethodHook.class);
    private static final TypeId<RposedAdditionalHookInfo> hookInfoTypeId
            = TypeId.get(RposedAdditionalHookInfo.class);
    private static final TypeId<RposedBridge.CopyOnWriteSortedSet> callbacksTypeId
            = TypeId.get(RposedBridge.CopyOnWriteSortedSet.class);
    private static final TypeId<RC_MethodHook.MethodHookParam> paramTypeId
            = TypeId.get(RC_MethodHook.MethodHookParam.class);
    private static final MethodId<RC_MethodHook.MethodHookParam, Void> setResultMethodId =
            paramTypeId.getMethod(TypeId.VOID, "setResult", TypeId.OBJECT);
    private static final MethodId<RC_MethodHook.MethodHookParam, Void> setThrowableMethodId =
            paramTypeId.getMethod(TypeId.VOID, "setThrowable", throwableTypeId);
    private static final MethodId<RC_MethodHook.MethodHookParam, Object> getResultMethodId =
            paramTypeId.getMethod(TypeId.OBJECT, "getResult");
    private static final MethodId<RC_MethodHook.MethodHookParam, Throwable> getThrowableMethodId =
            paramTypeId.getMethod(throwableTypeId, "getThrowable");
    private static final MethodId<RC_MethodHook.MethodHookParam, Boolean> hasThrowableMethodId =
            paramTypeId.getMethod(TypeId.BOOLEAN, "hasThrowable");
    private static final MethodId<RC_MethodHook, Void> callAfterCallbackMethodId =
            callbackTypeId.getMethod(TypeId.VOID, CALLBACK_METHOD_NAME_AFTER, paramTypeId);
    private static final MethodId<RC_MethodHook, Void> callBeforeCallbackMethodId =
            callbackTypeId.getMethod(TypeId.VOID, CALLBACK_METHOD_NAME_BEFORE, paramTypeId);
    private static final FieldId<RC_MethodHook.MethodHookParam, Boolean> returnEarlyFieldId =
            paramTypeId.getField(TypeId.BOOLEAN, "returnEarly");
    private static final TypeId<RposedBridge> xposedBridgeTypeId = TypeId.get(RposedBridge.class);
    private static final MethodId<RposedBridge, Void> logThrowableMethodId =
            xposedBridgeTypeId.getMethod(TypeId.VOID, "log", throwableTypeId);

    private FieldId<?, RposedAdditionalHookInfo> mHookInfoFieldId;
    ...
    private RposedAdditionalHookInfo mHookInfo;
    ...
    public void start(Member member, RposedAdditionalHookInfo hookInfo,
    ...
    private HookWrapper.HookEntity loadHookerClass(ClassLoader loader, String className) throws Exception {
        ...
        mHookClass.getMethod(METHOD_NAME_SETUP, Member.class, Method.class, RposedAdditionalHookInfo.class).invoke(null, mMember, mBackupMethod, mHookInfo);
    ...
    private void generateSetupMethod() {
        ...
        Local<RposedAdditionalHookInfo> hookInfo = code.getParameter(2, hookInfoTypeId);
...
//        }
    ...
    private void generateHookMethod() {
        ...
        Local<RposedAdditionalHookInfo> hookInfo = code.newLocal(hookInfoTypeId);
        Local<RposedBridge.CopyOnWriteSortedSet> callbacks = code.newLocal(callbacksTypeId);
        ...
        Local<RC_MethodHook> callback = code.newLocal(callbackTypeId);
        ...
        Local<RC_MethodHook.MethodHookParam> param = code.newLocal(paramTypeId);
        Local<Member> method = code.newLocal(memberTypeId);
        Local<Object> thisObject = code.newLocal(TypeId.OBJECT);
        Local<Object[]> args = code.newLocal(objArrayTypeId);
        ...
        // check RposedBridge.disableHooks flag

        FieldId<RposedBridge, Boolean> disableHooksField =
