...
import com.virjar.ratel.sandhook.utils.ParamWrapper;
import com.virjar.ratel.sandhook.wrapper.BackupMethodStubs;
...
public class HookStubManager {
    ...
    public static RposedAdditionalHookInfo[] additionalHookInfos;

    static {
        is64Bit = SandHook.is64Bit();
        Class stubClass = is64Bit ? MethodHookerStubs64.class : MethodHookerStubs32.class;
        stubSizes = (int[]) RposedHelpers.getStaticObjectField(stubClass, "stubSizes");
        Boolean hasBackup = (Boolean) RposedHelpers.getStaticObjectField(stubClass, "hasStubBackup");
        ...
        if (stubSizes != null && stubSizes.length > 0) {
            ...
            additionalHookInfos = new RposedAdditionalHookInfo[ALL_STUB];
        }
    }


    public static HookMethodEntity getHookMethodEntity(Member origin, RposedAdditionalHookInfo additionalHookInfo) {
        ...
        synchronized (HookStubManager.class) {
            ...
            if (hasStubBackup && !tryCompileAndResolveCallOriginMethod(entity.backup, stubMethodInfo.args, stubMethodInfo.index)) {
                Log.w(Constants.TAG, "internal stub <" + entity.hook.getName() + "> call origin compile failure, skip use internal stub");
                return null;
            } else {
    ...
    public static long hookBridge(int id, CallOriginCallBack callOrigin, long... stubArgs) throws Throwable {
        ...
        do {
            try {
                ((RC_MethodHook) snapshot[beforeIdx]).callBeforeHookedMethod(param);
            } catch (Throwable t) {
                Log.e(Constants.TAG, "error for call  hook with method beforeHookedMethod", t);
        ...
        do {
            ...
            try {
                ((RC_MethodHook) snapshot[afterIdx]).callAfterHookedMethod(param);
            } catch (Throwable t) {
                RposedBridge.log(t);
                Log.e(Constants.TAG, "error for call  hook with method afterHookedMethod", t);
     ...
     * @param origin
     * @param backup
     * @param additionalHookInfo
     * @param thiz
     * @param args
     * @return
     */
    public static Object hookBridge(Member origin, Method backup, RposedAdditionalHookInfo additionalHookInfo, Object thiz, Object... args) throws Throwable {
