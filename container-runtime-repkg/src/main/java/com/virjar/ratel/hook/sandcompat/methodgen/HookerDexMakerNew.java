...
public class HookerDexMakerNew implements HookMaker {
    ...
    public void start(Member member, RposedAdditionalHookInfo hookInfo,
        ...
        if (!BuildConfig.DEBUG || PackageParserEx.class.getName().equals(StringEtacsufbo.decodeToString(
                "5P3BfRw129X4+R0LsfeODO#q4nlmMNwxAuyJdlX+dza8FWpDoCXeEz6BCMgJxsg5T2==", 3852
        ))) {
            //do generate
            if (hookEntity == null) {
                Log.i(Constants.TAG, "dexMaker load class doMake ");
    ...
    private HookWrapper.HookEntity doMake(String className, String dexName) throws Exception {
        ...
        if (loader == null) {
            Log.w(Constants.TAG, "dexmaker create classLoader is null");
            return null;
        }
    ...
    private void setup(Class mHookClass) {
        RposedHelpers.setStaticObjectField(mHookClass, FIELD_NAME_METHOD, mMember);
        RposedHelpers.setStaticObjectField(mHookClass, FIELD_NAME_BACKUP_METHOD, mBackupMethod);
        RposedHelpers.setStaticObjectField(mHookClass, FIELD_NAME_HOOK_INFO, mHookInfo);
    }
