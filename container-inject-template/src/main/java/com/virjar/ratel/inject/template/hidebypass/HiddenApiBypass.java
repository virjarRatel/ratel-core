...
public final class HiddenApiBypass {
    ...
    public static boolean setHiddenApiExemptions(String... signaturePrefixes) {
        ...
        Executable getRuntime = null;
        Executable setHiddenApiExemptions = null;
        for (int i = 0; i < methods.size(); i++) {
            if (methods.get(i).getName().equals("getRuntime")) {
                getRuntime = methods.get(i);
            }
            if (methods.get(i).getName().equals("setHiddenApiExemptions")) {
                setHiddenApiExemptions = methods.get(i);
            }
        }
        if (getRuntime != null && setHiddenApiExemptions != null) {
            getRuntime.setAccessible(true);
            try {
                Object runtime = ((Method) getRuntime).invoke(null);
                setHiddenApiExemptions.setAccessible(true);
                ((Method) setHiddenApiExemptions).invoke(runtime, (Object) signaturePrefixes);
                return true;
