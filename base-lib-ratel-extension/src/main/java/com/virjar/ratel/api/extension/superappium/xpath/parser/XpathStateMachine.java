package com.virjar.ratel.api.extension.superappium.xpath.parser;
...
import com.virjar.ratel.api.extension.superappium.xpath.exception.NoSuchAxisException;
import com.virjar.ratel.api.extension.superappium.xpath.exception.XpathSyntaxErrorException;
import com.virjar.ratel.api.extension.superappium.xpath.model.Predicate;
        ...
        TAG {
            ...
            public void parse(XpathStateMachine stateMachine) throws XpathSyntaxErrorException {
                ...
                if (stateMachine.tokenQueue.peek() == '*') {
                    ...
                    stateMachine.xpathChain.getLast().setSelectParams(com.virjar.ratel.api.inspect.Lists.newArrayList("*"));
                ...
                if (stateMachine.tokenQueue.matches("@")) {// 遇到属性抽取动作
                    ...
                    if (stateMachine.tokenQueue.peek() == '*') {
                        ...
                        stateMachine.xpathChain.getLast()
                                .setSelectParams(com.virjar.ratel.api.inspect.Lists.newArrayList(attributeKey));
                    }
                    stateMachine.state = PREDICATE;
                    stateMachine.tokenQueue.consumeWhitespace();
                    return;
